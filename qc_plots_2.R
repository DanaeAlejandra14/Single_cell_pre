...
if (file.exists(f_seq)) {
  mini <- readr::read_csv(f_seq, show_col_types = FALSE)
  readr::write_csv(mini, file.path(out_dir, "00_qc_mini_summary.csv"))
  cat("# Mini summary (from qc_summary_cells_sequential.csv) written: ",
      file.path(out_dir, "00_qc_mini_summary.csv"), "\n")
} else {
  cat("# WARNING: qc_summary_cells_sequential.csv not found — using fallback\n")
  ...
  mini <- tibble(
    metric = c("cells_pre", "cells_after_counts_features", "cells_after_doublets", "cells_after_mt"),
    value  = c(total_pre, after_cf, after_doublets, after_mt_on_cf)
  )
  readr::write_csv(mini, file.path(out_dir, "00_qc_mini_summary.csv"))
  cat("# Fallback mini summary written (approximate)\n")
}
